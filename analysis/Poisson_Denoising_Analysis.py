...
def get_data(method):
    ...
    itts = []
    itts_std = []
    for gamma in tqdm([1,1.01, 1.02,1.03,1.04]):
        sub_scores = []
        sub_itts = []
        for rho in [4,5,6,7,8]:
            for image in images:
                y = np.array(proj(skimage.util.random_noise(image, mode='poisson'),[0,1]))
            ...
            combos.append(r"$\rho=$" + str(rho) + ", " + r"$\gamma=$" + str(gamma))
...
def get_baseline(method):
    ...
    for image in tqdm(images):
        ...
        if method == 'Non-Local Means':
            ...
            x = skimage.restoration.denoise_tv_chambolle(np.array(y), weight=0.2)
        elif method == 'BM3D':
            x = bm3d.bm3d(y,0.001)
        elif method == 'Median':
            x = ndimage.median_filter(y, size=6)
...
with open("../pickles/poisson_denoising.pickle", "rb") as f:
...
with open("../pickles/poisson_denoising.pickle", "wb") as f:
    pickle.dump((median_data, nlm_data, BM3D_data, TV_data), f)

plot_figs(median_data, median_baseline, nlm_data, nlm_baseline, BM3D_data, BM3D_baseline, TV_data, TV_baseline, 'poisson_denoising', "Poisson")