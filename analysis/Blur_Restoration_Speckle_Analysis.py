...
def get_data(method):
    ...
    itts = []
    itts_std = []
    for gamma in tqdm(np.arange(1, 1.4, 0.1), desc='gamma', leave=True):
        sub_scores = []
        sub_itts = []
        for rho in np.arange(0.02, 0.2, 0.02):
            for var in np.arange(0.01, 0.03, 0.01):
                for image in images:
                    z = np.array(proj(cv2.blur(image, (8,8)) ,[0,1]))
                    y = np.array(proj(skimage.util.random_noise(z, mode='speckle', var=var),[0,1]))
                    X = PlugPlayADMM_deblur(y, h, gamma=gamma, rho=rho, tol=0.005, method = method)
            ...
            combos.append(r"$\rho_0=$" + str(rho) + ", " + r"$\gamma=$" + str(gamma))
...
def get_baseline(method):
    ...
    """
    scores = []
    for var in tqdm(np.arange(0.07, 0.105, 0.01)):
        for image in images:
            z = np.array(proj(cv2.blur(image, (8,8)) ,[0,1]))
            y = np.array(proj(skimage.util.random_noise(z, mode='speckle', var=var),[0,1]))
            if method == 'Non-Local Means':
                ...
                x = skimage.restoration.denoise_tv_chambolle(np.array(y), weight=0.5)
            elif method == 'BM3D':
                x = bm3d.bm3d(y,0.01)
            elif method == 'Median':
...
with open("../pickles/speckle_blurr_restoration.pickle", "rb") as f:
...
with open("../pickles/speckle_blurr_restoration.pickle", "wb") as f:
    pickle.dump((median_data, nlm_data, BM3D_data, TV_data), f)

    

#plot results
plot_figs(median_data, median_baseline, nlm_data, nlm_baseline, BM3D_data, BM3D_baseline, TV_data, TV_baseline, 'speckle_blurr_restoration', "Speckle Deblurr")